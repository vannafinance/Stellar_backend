...
export const InfoCard = ({
  ...
  const renderItem = (item: InfoItem, idx: number, useAnimate = false) => (
    ...
    >
      ...
      <div
        className={`text-sm font-semibold flex-shrink-0 ${isDark ? "text-white" : "text-[#111111]"}`}
      >
        ...
        expandableSections.map((section, sectionIdx) => (
          ...
          >
            ...
            <motion.button
              ...
              } cursor-pointer w-full ${isDark ? "text-white" : "text-[#111111]"}`}
              whileTap={{ scale: 0.98 }}
              aria-expanded={expandedStates[section.title]}
              aria-controls={`section-${section.title}`}
              aria-label={`${expandedStates[section.title] ? "Collapse" : "Expand"} ${section.title}`}
            >
              {section.title}
              <motion.svg
                width="12"
