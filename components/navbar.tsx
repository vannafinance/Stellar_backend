...
export const Navbar = (props: Navbar) => {
  ...
  const handleWalletMouseEnter = () => {
    if (walletCloseTimeoutRef.current) {
      clearTimeout(walletCloseTimeoutRef.current);
      walletCloseTimeoutRef.current = null;
    }
  };

  const handleWalletMouseLeave = () => {
    walletCloseTimeoutRef.current = setTimeout(() => {
          ...
          {!address ? (
            ...
            <div className="relative">
              <div
                ...
                className={`cursor-pointer py-[12px] px-[24px] text-[16px] font-semibold rounded-[8px] h-full w-fit transition-colors hover:opacity-80 ${
                  isDark ? "bg-[#222222] text-white" : "bg-[#F4F4F4]"
                }`}
                title="Click to see wallet options"
              >
              ...
              <AnimatePresence>
                {isWalletDropdownOpen && (
                  <motion.div
                    ...
                    onMouseEnter={handleWalletMouseEnter}
                    onMouseLeave={handleWalletMouseLeave}
                    className={`absolute top-full right-0 mt-2 py-2 w-40 rounded-lg shadow-lg border ${
                      isDark 
                  ...
                  >
                    <div
                      ...
                      className={`px-4 py-2 text-sm cursor-pointer transition-colors hover:text-[#FF007A] ${
